...
//    $data = mysqli_real_escape_string($con, $data);
   ...
   if(isset($_POST['addcity']) && $_SERVER['REQUEST_METHOD'] == 'POST'){
//        $id = cleanup($_POST['id']);
    ...
    //        echo "<script>alert('Please Fill All Fields Properly'); window.location.href='insert-inventory.php';</script>";
            ...
            $checkcity = "SELECT `name` from `city` WHERE `name`=:name";
            $stmt2 = $con->prepare($checkcity);
            $stmt2->execute(["name"=>$city_name]);
        ...
        if($stmt){
            ...
            $operation = "Insert";
            $comment = $_SESSION['admin_username']." has inserted $city_name in city list at $datenow";
