...
//    $data = mysqli_real_escape_string($con, $data);
   ...
   if(isset($_POST['updateuser']) && $_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if(empty($username) || empty($pwd) || empty($usertype)){
            echo "mandatory";
    //        echo "<script>alert('Please Fill All Fields Properly'); window.location.href='insert-inventory.php';</script>";
        }
        else{
            $updateqry="UPDATE `admin` SET `username`=:username,
            `password`=:password,`admin_type`=:admin_type WHERE `id`=:id";
                $stmt = $con->prepare($updateqry);
                $stmt->execute(["username"=>$username,
                "password"=>$pwd,"admin_type"=>$usertype,"id"=>$id
                ]);
        if($stmt){
